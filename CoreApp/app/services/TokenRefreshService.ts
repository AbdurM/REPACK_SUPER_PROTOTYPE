...
class TokenRefreshService {
  ...
  async getValidToken(): Promise<TokenData | null> {
    ...
    if (this.refreshPromise) {
      // console.log(
      //   '[TokenRefreshService] Waiting for existing refresh operation...',
      // );
      return this.refreshPromise;
    }

    // No refresh in progress - this caller will initiate it
    // console.log('[TokenRefreshService] Initiating new refresh operation...');
    ...
    this.refreshPromise = this.performTokenRefresh()
      .then(tokenData => {
        // console.log('[TokenRefreshService] Refresh completed successfully');
        return tokenData;
      })
      .catch(error => {
      ...
      .finally(() => {
        ...
        // console.log('[TokenRefreshService] Clearing refresh promise');
  ...
  private async performTokenRefresh(): Promise<TokenData | null> {
    try {
      ...
      if (!loginData?.refreshToken) {
        // console.error('[TokenRefreshService] No refresh token found');
        await this.handleRefreshFailure();
        return null;
      }

      // Step 2: Call the refresh endpoint
      // console.log('[TokenRefreshService] Calling refresh token API...');
      const result = await refreshAuthToken(loginData.refreshToken);

      if (!result || !result.accessToken) {
        // console.error(
        //   '[TokenRefreshService] Refresh API returned invalid data',
        // );
        await this.handleRefreshFailure();
        return null;
      }
      ...
      // console.error(
      //   '[TokenRefreshService] Exception during token refresh:',
      //   error,
      // );
      await this.handleRefreshFailure();
      return null;
  ...
  private async handleRefreshFailure(): Promise<void> {
    // console.log(
    //   '[TokenRefreshService] Handling refresh failure - logging out user',
    // );
  ...
  async logout(logoutType: LogoutType = LogoutTypes.hard): Promise<void> {
    // console.log(`[TokenRefreshService] Logout triggered (${logoutType})`);
  ...
  clearRefreshState(): void {
    // console.warn('[TokenRefreshService] Force clearing refresh state');
    this.refreshPromise = null;
    this.isRefreshing = false;
  }
}
